...
def plot_KDE(ax, data, colors, fill, levels = [0.05, 0.32, 1],  ls='solid', bw_adjust=1.5):
    # levels = [0.05, 0.32, 1]
    lw = 4.0
...
def plot_1D_quantiles(ax, y, colors, ls):
    ...
    if ls == 'filled':
        ax.fill_between(x, contours[:,0], contours[:,3], color=colors, alpha = 0.4)
        ax.fill_between(x, contours[:,1], contours[:,2], color=colors, alpha = 0.8)
    else:
        lw = 1.5 if ls != 'dotted' else 1.5
        ax.plot(x, contours[:,0], color=colors, ls=ls, lw=lw)
        ax.plot(x, contours[:,1], color=colors, ls=ls, lw=lw)
        ax.plot(x, contours[:,2], color=colors, ls=ls, lw=lw)
        ax.plot(x, contours[:,3], color=colors, ls=ls, lw=lw)
...
def double_interval_legend(color, lw=4):
    return (custom_patch(color[0], alpha = 0.4, lw=lw), custom_line(color[1], lw=lw))
...
c_dark_energy = '#E76F51'
c_bosonic = 'xkcd:violet'
c_fermionic = 'xkcd:azure'
