...
minpres_prior = np.load(script_dir / 'DE/Dark_energy_prior_minpres_total.npy')
maxpres_prior = np.load(script_dir / 'DE/Dark_energy_prior_maxpres_total.npy')


prior_contours = minpres_prior,maxpres_prior
print(np.shape(prior_contours))


minpres_posterior = np.load(script_dir / 'DE/Dark_energy_posterior_minpres_total.npy')
maxpres_posterior = np.load(script_dir / 'DE/Dark_energy_posterior_maxpres_total.npy')
posterior_contours = minpres_posterior,maxpres_posterior

print(np.shape(posterior_contours))
...
Bosonic_minpres_posterior = np.load('BDM/Bosonic_posterior_minpres_total.npy')
Bosonic_maxpres_posterior = np.load('BDM/Bosonic_posterior_maxpres_total.npy')
Bosonic_posterior_contours = Bosonic_minpres_posterior,Bosonic_maxpres_posterior

# pressures = np.load('BDM/Bosonic_posterior_pressures.npy')
# Bosonic_posterior_contours = calc_bands(energydensities_ADM, pressures)

print(np.shape(Bosonic_posterior_contours))


Fermionic_minpres_posterior = np.load('FDM/Fermionic_posterior_minpres.npy')
Fermionic_maxpres_posterior = np.load('FDM/Fermionic_posterior_maxpres.npy')
Fermionic_posterior_contours = Fermionic_minpres_posterior,Fermionic_maxpres_posterior


# pressures = np.load('POSTERIOR_BARYONIC_REAL_pressures.npy')
...
def mass_radius_posterior_plot(root_name_1,root_name_2,root_name_3, root_name_4,ax = None):
    scatter_1 = np.loadtxt(root_name_1 + 'MR_prpr.txt')
    # scatter_ADM = []
    ...
    scatter_2 = np.loadtxt(root_name_2 + 'MR_prpr.txt')
        # scatter_prior = []
        ...
        # scatter_prior = np.array(scatter_prior)
    ...
    scatter_3= np.loadtxt(root_name_3 + 'MR_prpr.txt')
    ...
    scatter_4 = np.loadtxt(root_name_4 + 'MR_prpr.txt')
...
contours_min = np.load('B/Baryonic_posterior_minpres.npy')
contours_max = np.load('B/Baryonic_posterior_maxpres.npy')
minpres_ppNI = np.log10(contours_min)
maxpres_ppNI = np.log10(contours_max)
...
#maxpres_ppNI = np.log10(np.load(root_name_B + 'minpres.npy'))
#maxpres_ppNI = np.log10(np.load(root_name_B + 'maxpres.npy'))


posterior_contours_min = posterior_contours[0]
posterior_contours_max = posterior_contours[1]
minpres_posterior = np.log10(posterior_contours_min)
maxpres_posterior = np.log10(posterior_contours_max)

Bosonic_posterior_contours_min = Bosonic_posterior_contours[0]
Bosonic_posterior_contours_max = Bosonic_posterior_contours[1]
Bosonic_minpres_posterior = np.log10(Bosonic_posterior_contours_min)
Bosonic_maxpres_posterior = np.log10(Bosonic_posterior_contours_max)
...
Fermionic_minpres_posterior = np.log10(Fermionic_posterior_contours_min)
Fermionic_maxpres_posterior = np.log10(Fermionic_posterior_contours_max)
...
energydensities = np.logspace(14.2, 16, 50)
#pressures_prior = np.load('FERMIONIC_REAL_DATA_PRIOR_Pressure_array.npy')
...
minpres_posterior = np.load('DE/Dark_energy_posterior_minpres_baryon.npy')
maxpres_posterior = np.load('DE/Dark_energy_posterior_maxpres_baryon.npy')
posterior_contours = minpres_posterior,maxpres_posterior

print(np.shape(posterior_contours))

Bosonic_minpres_posterior = np.load('BDM/Bosonic_posterior_minpres_baryon.npy')
Bosonic_maxpres_posterior = np.load('BDM/Bosonic_posterior_maxpres_baryon.npy')
Bosonic_posterior_contours = Bosonic_minpres_posterior,Bosonic_maxpres_posterior


Fermionic_minpres_posterior = np.load('FDM/Fermionic_posterior_minpres_baryon.npy')
Fermionic_maxpres_posterior = np.load('FDM/Fermionic_posterior_maxpres_baryon.npy')
Fermionic_posterior_contours = Fermionic_minpres_posterior,Fermionic_maxpres_posterior
...
ax.set_xticks(np.arange(14.25, 15.24,.05),minor =True)
ax.set_ylim(33,36)
