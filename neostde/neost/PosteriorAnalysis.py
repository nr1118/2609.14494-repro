...
def compute_auxiliary_data(root_name, EOS, variable_params, static_params, chirp_masses, dm = False):
    ...
    if dm == False:
        ...
        energydensities_dm = np.logspace(10, 18, 200)
...
def compute_table_data_de(root_name, EOS, variable_params, static_params):
    ...
    try:
        ...
        for i in tqdm(range(0, len(ewposterior), 1)):
            ...
            eps = np.linspace(EOS.rho_plus/rho_ns +0.1, EOS.max_edsc_de, 40)*rho_ns
            M = np.zeros(len(eps))
            R = np.zeros(len(eps))
            for j, e in enumerate(eps):
...
def compute_auxiliary_data_de2(root_name, EOS, variable_params, static_params, prior = False):
    ...
    for i in tqdm(range(0, len(ewprior), 1)):
        ...
        eps = np.linspace(EOS.rho_plus/rho_ns +0.1, EOS.max_edsc_de, 40)*rho_ns
        rhoc = np.random.rand() *(EOS.max_edsc_de - EOS.rho_plus/rho_ns - 0.1) + EOS.rho_plus/rho_ns +0.1 #the 0.1 is here to avoid be directly up to the transition density
...
def compute_auxiliary_data_ADM(root_name, EOS, variable_params, static_params, prior = False):
    ...
    for i in tqdm(range(0, len(ewprior), 1)):
        ...
        # densities = np.logspace(14,14.16)
        # print(max(np.log10(eos_dm(densities))), max(np.log10(EOS.eos(densities))))
