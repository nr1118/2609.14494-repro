...
def solveTOVdm(double rhobcent, double rhodmcent, eos_epsb, eos_presb, eos_epsdm, eos_presdm, dm_halo, two_fluid_tidal,double atol,
    ...
    if(stateTOV[0] > stateTOV[1]): 
        ...
        while (stateTOV_single[0]>Pmin):
            ...
            Array = np.append(Array, np.array([[r,stateTOV_single[1],eps,stateTOV_single[0],0,0]]), axis=0) #added 10/24 #epsdm, stateTOV[1]
    ...
    if(stateTOV[1] > stateTOV[0]): 
        ...
        if dm_halo == False:
            ...
            while (stateTOV_single[0]>Pmin and dr-r>1e-14): ##with halos that are very large it seems setting >0 is the way to go as opposed to using Pmin## and dr-r>1e-14
                status = gsl_odeiv2_driver_apply(d_single, &r, dr, stateTOV_single)
                if (status != GSL_SUCCESS):
                    printf ("error, return value=%d\n", status)
                    break
                ...
                #Gtt = np.append(Gtt, np.array([[r, stateTOV_single[2]]]), axis=0)
                dr = r + step * pow(pow(stateTOV_single[1],-1) * dmdr - pow(stateTOV_single[0],-1)*dpdr, -1)
                Array = np.append(Array, np.array([[r,stateTOV_single[1],0,0, eps,stateTOV_single[0]]]), axis=0) #added 10/24 #epsb, stateTOV[0]
    ...
    return Mb, Rns, Mdm_core, Mdm_halo, Rdm_core, Rdm_halo,tidal, Array