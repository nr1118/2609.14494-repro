...
class BaseEoS():
    ...
    def update(self, eos_params, max_edsc=True,max_edsc_admixed=False, max_edsc_de = False):
        """
        ...
        if max_edsc is True:
            ...
            self.max_edsc = 0.0
        if max_edsc_admixed is True:
            self.find_max_edsc_admixed()
        else:
            self.max_edsc_admixed = 0.0
    ...
    def find_max_edsc(self):
        ...
        eds_c = np.logspace(min_edsc0, np.log10(maximum), 75) # g/cm^3
    ...
    # Find maximum central energy density
    def find_max_edsc_admixed(self):

        min_edsc0 = 14.3

        eds = np.logspace(15, np.log10(1e26), 1000) #same as above

        eos_dm  = UnivariateSpline(self.energydensities_dm, self.pressures_dm, k=1, s=0, ext=1)
        dpde = eos_dm.derivative(1)
        cs = dpde(eds)/c**2
        acausal = 1.
        ...
        # index_max_M = np.argmax(Ms[:,0])
        # self.Radius_max_M = Ms[:,1][index_max_M]
        self.max_edsc_dm = max(eds_c)
        self.max_edsc_admixed = self.max_edsc + self.max_edsc_dm
        #self.min_edsc = 10**(min_edsc0)
        # if Ms[:,0][-1] > 0.9:
        #     self.min_edsc = min(eds_c[Ms[:,0] > 0.9])
        # self.centraleds = eds_c
    ...
    def find_max_edsc_de(self):
        ...
        self.min_edsc_de = self.rho_plus/rho_ns + 0.005
        #if Ms[:,0][-1] > 0.9:
            #self.min_edsc_de = min(eds_c[Ms[:,0] > 0.5])/rho_ns
