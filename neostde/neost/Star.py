...
class Star():
    ...
    def solve_structure(self, eps, pres,eps_dm = None,pres_dm = None,dm_halo = False,two_fluid_tidal = False, atol=1e-6, rtol=1e-4, hmax=1000., step=0.2):
        """
        ...
        if self.enthalpy is True:
            ...
            if self.epscent_dm == 0.0:
                if self.dark_energy == False:
                    ...
                    self.Mb, self.Mdm_core, self.radius_dm_core, self.Rns, self.tidal, self.Gtt = solveTOVde(self.epscent, self.eps_plus, self.alpha, eps, pres, eps_dm, pres_dm, atol, rtol, hmax, step)


            else:
                self.Mb, self.Rns, self.Mdm_core, self.Mdm_halo, self.radius_dm_core, self.radius_dm_halo, self.tidal, self.Array = solveTOVdm(self.epscent, self.epscent_dm, eps, pres, eps_dm, pres_dm, dm_halo,two_fluid_tidal, atol, rtol, hmax, step)
