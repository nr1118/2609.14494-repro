...
EOS.update({'gamma1':2.3, 'gamma2':4., 'gamma3':2.6, 'rho_t1':1.8, 'rho_t2':4, 'A_param': 0.3, 'alpha': 0.4, 'rho_plus': 2.78, 'ceft': 2.6}, max_edsc=True)


central_densities = np.linspace(EOS.rho_plus/rho_ns +0.1, pow(10,16)/rho_ns, 75)*rho_ns
...
lns1 = ax.plot(MR[:,1], MR[:,0],label='DE admixed neutron star', lw=2.5,color = '#00B1B7')
...
plt.tight_layout()
plt.show()
plt.savefig('chains/' + 'MR_example.png')
